    ...
    const updateProfile = async () => {
        try {
            ...
            if (data.success) {
                ...
                setIsEdit(false)
                setProfileData(prev => ({ ...prev, available: updateData.available }));
                getProfileData()
        ...
        <div>
            <div className='flex flex-col gap-4 m-5'>
                ...
                <div className='flex-1 border border-stone-200 rounded-lg p-8 py-7 bg-white'>
                    ...
                    <p className='flex items-center gap-2 text-5xl font-medium text-gray-700'>{profileData.name}</p>
                    ...
                    <div className='flex gap-1 pt-2'>
                        <input type="checkbox" checked={profileData.available} onChange={() => {
                            if (isEdit) {
                    ...
                    {
                        isEdit
                            ? <button onClick={updateProfile} className='px-4 mt-4 py-1 border border-primary text-sm rounded-full hover:bg-primary hover:text-white transition-all'>Save</button>
                            : <button onClick={() => setIsEdit(true)} className='px-4 py-1 mt-4 border border-primary text-sm rounded-full hover:bg-primary hover:text-white transition-all'>Edit</button>
                    }
                </div>
            </div>
        </div>

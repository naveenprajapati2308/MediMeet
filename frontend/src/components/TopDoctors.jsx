        ...
        <div className='flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10'>
            ...
            <div className='w-full grid grid-cols-auto gap-4 pt-5 gap-y-6 px-6 sm:px-0'>
                {doctors.slice(0, 10).map((item, index) => (
                    <div onClick={() => { navigate(`/appointment/${item._id}`); scrollTo(0, 0) }} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-20px] transition-all duration-500 ' key={index}>
